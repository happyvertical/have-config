...
- Run `/review-cycle` before opening PRs. Inherit its review rules, including no `ultrareview`, at least 15-minute review timeouts, and up to the configured round cap.
...
8. Read repository instructions and shipping context in every included repository:
   ...
   - `.github/workflows/*`
9. Look for `## Shipping Notes` in repo-level `AGENTS.md`. If present, treat it as the source of repo-specific shipping commands, docs requirements, CI quirks, release notes, PR expectations, and known flaky checks.
...
1. Inspect the current diff and classify the change:
   ...
   - docs-only
   - tests-only
2. Run the repo's normal formatting/linting path when configured. Prefer the commands documented in `Shipping Notes`, `AGENTS.md`, package scripts, Makefiles, taskfiles, or README.
...
After PRs exist, watch CI in dependency order:
